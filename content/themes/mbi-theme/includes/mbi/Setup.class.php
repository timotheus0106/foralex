...
Class Setup {
	...
	protected function __construct() {
		...
		// if(Settings::getInstance()->get_option('dynamic_images') === true) {
		// 	$this->images_on_demand();
		// }

		if(count(Settings::getInstance()->get_option('acf_tinymce')) > 0) {
	...
	public function on_init() {
		...
		// Add Tags to Page
		if(Settings::getInstance()->get_option('page_tags') === true) {
		...
		*/
		if(Settings::getInstance()->get_option('beautify_search') === true) {
		...
		// if(count(Settings::getInstance()->get_option('artdirected_images')) > 0) {
		// 	$this->initArtdirectedImages();
		// }

		// if(count(Settings::getInstance()->get_option('single_images')) > 0) {
	...
	public function on_after_setup_theme() {
		...
		if(Settings::getInstance()->get_option('remove_image_attributes')){
			add_filter('post_thumbnail_html', array($this, 'remove_thumbnail_dimensions'), 10);
		}

		// Register Menus
		register_nav_menus(Settings::getInstance()->get_option('menus'));

		// Remove Comments
		if(Settings::getInstance()->get_option('comments') === false) {
			$this->remove_comments();
		}

		// remove widgets
		if(Settings::getInstance()->get_option('widgets') === false) {
	...
	*/
	...
	public function rewrite_rules() {
		...
		foreach(Settings::get_option('rewrite_rules') as $rule) {
	...
	public function initArtdirectedImages() {
	  ...
	  foreach(Settings::getInstance()->get_option('artdirected_images') as $name => $options) {
	...
	public function initSingleImages(){
		...
		foreach(Settings::getInstance()->get_option('single_images') as $name => $options) {
	...
	*/
	...
	function acf_tiny_mce($toolbars) {
		...
		foreach(Settings::getInstance()->get_option('acf_tinymce') as $bar => $values) {
